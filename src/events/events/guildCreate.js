...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`LeunelBot has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const LeunelBot = guild.roles.cache.find(r => r.name === '#Leunel');
    if (LeunelBot) await guild.me.roles.add(LeunelBot);
  } catch (err) {
    client.logger.error(err.message);
  }
